...
class Cube:
    ...
    def determineMove(self,direction,location):
        if direction ==1:
            action = right_left_moves[location[1]]
            action[0](action[1])
        elif direction ==2:
            action = right_left_moves[location[1]]
            action[0](action[1]*-1)
        elif direction ==3:
            action = up_down_moves[location[0]]
            action[0](action[1]*-1)
        elif direction ==4:
            action = up_down_moves[location[0]]
            action[0](action[1])
...
##to-do for v8
##create algorithm for quick moves
##eventually scan the cube to find the quickest way to solve based on orientation and play back moves
##temp fix for getting location function ?? middle top squares not working can use shapes later?
##animations?
if __name__ == '__main__':
    ...
    while True:
        for event in pygame.event.get():
            ...
            ##keyboard controls for the rubix cube   
            ...
            ##mouse controls for the rubix cube
                ...
                if event.type==pygame.MOUSEBUTTONUP:
                    ...
                    ##if outside cube then rotate whole cube
                    if location == [-1,-1]:
                        if direction==1:
                            cube.rotateCube(H,1)
                        elif direction==2:
                            ...
                            cube.rotateCube(V,1)
                    ##topface check angled directions       
                    elif location[0] == 1 or location[0] ==0:
                        if (location == [0,0] and direction ==2) or (location == [0,1] and direction==4):
                            cube.determineMove(direction, location)
                        elif direction ==2 or direction ==4:
                            cube.determineMove(direction,location) 
                            cube.determineMove(angle_direction,location)
                        else:
                            cube.determineMove(angle_direction,location)
                    else:
                        cube.determineMove(direction,location)
